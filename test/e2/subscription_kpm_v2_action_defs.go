...
// TestActionDefinitionKpmV2 tests e2 subscription and subscription delete procedures using kpm version 2
func (s *TestSuite) TestActionDefinitionKpmV2(t *testing.T) {
	sim := utils.CreateRanSimulatorWithNameOrDie(t, "kpm-v2-action-definitions")
	assert.NotNil(t, sim)

	e2Client := getE2Client(t, "kpm-v2-action-definitions-test")
