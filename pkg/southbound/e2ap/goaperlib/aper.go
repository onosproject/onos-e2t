		...
		if pd.canonicalOrdering {
			...
			if len(choiceMap) > 1 {
				...
				for j := 1; j <= len(choiceMap); j++ {
					ie, ok := choiceMap[j]
					if !ok {
						return errors.NewInvalid("Expected an index %d in a Regular Choice map with %s", j, params.oneofName)
					}
				...
				if !ok {
					return errors.NewInvalid("Expected a Regular Choice map %s to have index %d", params.oneofName, choiceIdx)
				}
				choiceStruct = reflect.New(choiceType)
				if v.CanSet() {
					v.Set(choiceStruct)
				...
				if pd.choiceCanBeExtended {
					...
					if !ok {
						return errors.NewInvalid("Expected a Regular Choice map %s to have index %d", params.oneofName, 1)
					}
					choiceStruct = reflect.New(choiceType)
					if v.CanSet() {
						v.Set(choiceStruct)
