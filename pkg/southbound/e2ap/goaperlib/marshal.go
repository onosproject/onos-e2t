...
func (pd *perRawBitData) appendCanonicalChoiceIndex(canonicalChoiceMap map[int64]reflect.Type, v reflect.Value, params fieldParameters) error {
	...
	threadedBytes := &perRawBitData{[]byte(""), 0, -1, false, false}
...
func (pd *perRawBitData) appendOpenType(v reflect.Value, params fieldParameters) error {
	...
	pdOpenType := &perRawBitData{[]byte(""), 0, -1, false, false}
		...
		for i := 0; i < structType.NumField(); i++ {
			...
			if structParams[fieldIdx].oneofName != "" {
				if params.canonicalOrder {
					...
					if err := pd.appendCanonicalChoiceIndex(canonicalChoiceMap, reflect.ValueOf(v.Field(i).Interface()), tempParams); err != nil {
						return err
					}
				} else {
					...
					if len(choiceMap) > 1 {
						...
						for j := 1; j <= len(choiceMap); j++ {
							ie, ok := choiceMap[j]
							if !ok {
								return errors.NewInvalid("Expected an index %d in a Regular Choice map with %s", j, choiceType)
							}
						...
						if err := pd.appendChoiceIndex(present, structParams[fieldIdx].valueExtensible, structParams[fieldIdx].fromChoiceExt, ieNotInExt, len(choiceMap)); err != nil {
							return err
						}
					} else {
...
func MarshalWithParams(val interface{}, params string) ([]byte, error) {
	pd := &perRawBitData{[]byte(""), 0, -1, false, false}
