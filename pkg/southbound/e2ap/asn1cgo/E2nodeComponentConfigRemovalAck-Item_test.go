...
func createE2nodeComponentConfigRemovalAckItemMsg() (*e2ap_pdu_contents.E2NodeComponentConfigRemovalAckItem, error) {

	e2nccai := e2ap_pdu_contents.E2NodeComponentConfigRemovalAckItem{
...
func Test_xerEncodingE2nodeComponentConfigRemovalAckItem(t *testing.T) {

	e2nccaai, err := createE2nodeComponentConfigRemovalAckItemMsg()
	assert.NilError(t, err, "Error creating E2nodeComponentConfigRemovalAckItem PDU")

	xer, err := xerEncodeE2nodeComponentConfigRemovalAckItem(e2nccaai)
	assert.NilError(t, err)
	t.Logf("E2nodeComponentConfigRemovalAckItem XER\n%s", string(xer))

	result, err := xerDecodeE2nodeComponentConfigRemovalAckItem(xer)
	assert.NilError(t, err)
	assert.Assert(t, result != nil)
	t.Logf("E2nodeComponentConfigRemovalAckItem XER - decoded\n%v", result)
...
func Test_perEncodingE2nodeComponentConfigRemovalAckItem(t *testing.T) {

	e2nccaai, err := createE2nodeComponentConfigRemovalAckItemMsg()
	assert.NilError(t, err, "Error creating E2nodeComponentConfigRemovalAckItem PDU")

	per, err := perEncodeE2nodeComponentConfigRemovalAckItem(e2nccaai)
	assert.NilError(t, err)
	t.Logf("E2nodeComponentConfigRemovalAckItem PER\n%v", hex.Dump(per))

	result, err := perDecodeE2nodeComponentConfigRemovalAckItem(per)
	assert.NilError(t, err)
	assert.Assert(t, result != nil)
	t.Logf("E2nodeComponentConfigRemovalAckItem PER - decoded\n%v", result)
	assert.Equal(t, e2nccaai.GetE2NodeComponentConfigurationAck().GetUpdateOutcome().Number(), result.GetE2NodeComponentConfigurationAck().GetUpdateOutcome().Number())
	assert.Equal(t, e2nccaai.GetE2NodeComponentConfigurationAck().GetFailureCause().GetE2Node().Number(), result.GetE2NodeComponentConfigurationAck().GetFailureCause().GetE2Node().Number())
	assert.Equal(t, e2nccaai.GetE2NodeComponentId().GetE2NodeComponentInterfaceTypeNg().GetAmfName().GetValue(), result.GetE2NodeComponentId().GetE2NodeComponentInterfaceTypeNg().GetAmfName().GetValue())
	assert.Equal(t, e2nccaai.GetE2NodeComponentInterfaceType().Number(), result.GetE2NodeComponentInterfaceType().Number())
