...
func NewE2Conn(nodeID topoapi.ID, plmnID string, nodeIdentity *types.E2NodeIdentity, conn e2.ServerConn,
	...
	serviceModels map[string]*topoapi.ServiceModelInfo, ranFunctions map[e2smtypes.OID]RANFunction, e2Cells []*topoapi.E2Cell, now time.Time) *E2Conn {
...
func (c *E2Conn) ricIndication(ctx context.Context, request *e2appducontents.Ricindication) error {
	...
	if !ok {
		return errors.NewNotFound("stream with ID %s not found", streamID)
	}
