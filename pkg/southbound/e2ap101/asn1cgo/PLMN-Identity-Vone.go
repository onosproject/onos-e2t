...
func xerEncodePlmnIdentity(plmnIdentity *e2ap_commondatatypes.PlmnIdentity) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_PLMN_Identity_Vone, unsafe.Pointer(plmnIdentityCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodePlmnIdentity() %s", err.Error())
	}
	return bytes, nil
...
func perEncodePlmnIdentity(plmnIdentity *e2ap_commondatatypes.PlmnIdentity) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_PLMN_Identity_Vone, unsafe.Pointer(plmnIdentityCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodePlmnIdentity() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodePlmnIdentity(bytes []byte) (*e2ap_commondatatypes.PlmnIdentity, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_PLMN_Identity_Vone)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodePlmnIdentity((*C.PLMN_Identity_Vone_t)(unsafePtr))
}

func perDecodePlmnIdentity(bytes []byte) (*e2ap_commondatatypes.PlmnIdentity, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_PLMN_Identity_Vone)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newPlmnIdentity(plmnIdentity *e2ap_commondatatypes.PlmnIdentity) (*C.PLMN_Identity_Vone_t, error) {
...
func decodePlmnIdentity(plmnIdentityC *C.PLMN_Identity_Vone_t) (*e2ap_commondatatypes.PlmnIdentity, error) {
