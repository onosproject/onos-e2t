...
func newRicActionNotAdmittedItem(raai *e2appducontents.RicactionNotAdmittedItem) (*C.RICaction_NotAdmitted_Item_Vone_t, error) {
	cause, err := newCause(raai.GetCause())
	if err != nil {
		return nil, fmt.Errorf("newCause() %s", err.Error())
	}
	raaiC := C.RICaction_NotAdmitted_Item_Vone_t{
...
func decodeRicActionNotAdmittedItemBytes(ranaiBytes []byte) (*e2appducontents.RicactionNotAdmittedItem, error) {
	raaiC := C.RICaction_NotAdmitted_Item_Vone_t{
		...
		cause: C.CauseVone_t{
			present: C.CauseVone_PR(binary.LittleEndian.Uint64(ranaiBytes[8:])),
		},
	}
...
func decodeRicActionNotAdmittedItem(ranaiC C.RICaction_NotAdmitted_Item_Vone_t) (*e2appducontents.RicactionNotAdmittedItem, error) {
