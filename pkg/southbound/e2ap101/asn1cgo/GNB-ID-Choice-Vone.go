...
func newGnbIDChoice(gnbIDCh *e2apies.GnbIdChoice) (*C.GNB_ID_Choice_Vone_t, error) {
	var pr C.GNB_ID_Choice_Vone_PR

	choiceC := [48]byte{}

	switch choice := gnbIDCh.GetGnbIdChoice().(type) {
	case *e2apies.GnbIdChoice_GnbId:
		pr = C.GNB_ID_Choice_Vone_PR_gnb_ID
...
func decodeGnbIDChoice(gnbIDC *C.GNB_ID_Choice_Vone_t) (*e2apies.GnbIdChoice, error) {
	result := new(e2apies.GnbIdChoice)

	switch gnbIDC.present {
	case C.GNB_ID_Choice_Vone_PR_gnb_ID:
