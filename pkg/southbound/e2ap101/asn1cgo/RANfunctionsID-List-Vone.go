...
func xerEncodeRanFunctionsIDList(rfl *e2appducontents.RanfunctionsIdList) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_RANfunctionsID_List_Vone, unsafe.Pointer(rflC))
	if err != nil {
		return nil, err
	}
	return bytes, nil
...
func perEncodeRanFunctionsIDList(rfl *e2appducontents.RanfunctionsIdList) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_RANfunctionsID_List_Vone, unsafe.Pointer(rflC))
	if err != nil {
		return nil, err
	}
	return bytes, nil
...
func xerDecodeRanFunctionsIDList(xer []byte) (*e2appducontents.RanfunctionsIdList, error) {
	unsafePtr, err := decodeXer(xer, &C.asn_DEF_RANfunctionsID_List_Vone)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newRanFunctionsIDList(rfIDl *e2appducontents.RanfunctionsIdList) (*C.RANfunctionsID_List_Vone_t, error) {
	rfIDlC := new(C.RANfunctionsID_List_Vone_t)
...
func decodeRanFunctionsIDListBytes(ranFunctionIDListChoice [112]byte) (*e2appducontents.RanfunctionsIdList, error) {
	...
	rfIDlC := C.RANfunctionsID_List_Vone_t{
		list: C.struct___141{
			array: array,
			size:  size,
			count: count,
...
func decodeRanFunctionsIDList(rfIDlC *C.RANfunctionsID_List_Vone_t) (*e2appducontents.RanfunctionsIdList, error) {
