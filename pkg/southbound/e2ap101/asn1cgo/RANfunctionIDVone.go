...
func decodeRanFunctionIDBytes(ranFunctionIDCbytes []byte) *e2apies.RanfunctionId {
	rfC := (C.RANfunctionIDVone_t)(binary.LittleEndian.Uint64(ranFunctionIDCbytes[0:8]))

	return decodeRanFunctionID(&rfC)
}

func decodeRanFunctionID(ranFunctionIDC *C.RANfunctionIDVone_t) *e2apies.RanfunctionId {
