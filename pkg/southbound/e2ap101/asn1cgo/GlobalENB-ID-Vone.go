...
func xerEncodeeNBID(gnbID *e2apies.GlobalEnbId) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_GlobalENB_ID_Vone, unsafe.Pointer(gnbIDC))
	if err != nil {
		return nil, err
	}
	return bytes, nil
...
func perEncodeeNBID(gnbID *e2apies.GlobalEnbId) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_GlobalENB_ID_Vone, unsafe.Pointer(gnbIDC))
	if err != nil {
		return nil, err
	}
...
func newGlobaleNBID(id *e2apies.GlobalEnbId) (*C.GlobalENB_ID_Vone_t, error) {
...
func decodeGlobalEnbID(globalEnbID *C.GlobalENB_ID_Vone_t) (*e2apies.GlobalEnbId, error) {
