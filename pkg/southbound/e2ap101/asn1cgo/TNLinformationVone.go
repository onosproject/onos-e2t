...
func xerEncodeTnlinformation(tnlinformation *e2ap_ies.Tnlinformation) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_TNLinformationVone, unsafe.Pointer(tnlinformationCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeTnlinformation() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeTnlinformation(tnlinformation *e2ap_ies.Tnlinformation) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_TNLinformationVone, unsafe.Pointer(tnlinformationCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodeTnlinformation() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodeTnlinformation(bytes []byte) (*e2ap_ies.Tnlinformation, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_TNLinformationVone)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeTnlinformation((*C.TNLinformationVone_t)(unsafePtr))
}

func perDecodeTnlinformation(bytes []byte) (*e2ap_ies.Tnlinformation, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_TNLinformationVone)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newTnlinformation(tnlinformation *e2ap_ies.Tnlinformation) (*C.TNLinformationVone_t, error) {

	tnlinformationC := C.TNLinformationVone_t{}
...
func decodeTnlinformation(tnlinformationC *C.TNLinformationVone_t) (*e2ap_ies.Tnlinformation, error) {
