...
func xerEncodeRanFunctionsList(rfl *e2appducontents.RanfunctionsList) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_RANfunctions_List_Vone, unsafe.Pointer(rflC))
	if err != nil {
		return nil, err
	}
	return bytes, nil
...
func perEncodeRanFunctionsList(rfl *e2appducontents.RanfunctionsList) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_RANfunctions_List_Vone, unsafe.Pointer(rflC))
	if err != nil {
		return nil, err
	}
	return bytes, nil
...
func xerDecodeRanFunctionList(xer []byte) (*e2appducontents.RanfunctionsList, error) {
	unsafePtr, err := decodeXer(xer, &C.asn_DEF_RANfunctions_List_Vone)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newRanFunctionsList(rfl *e2appducontents.RanfunctionsList) (*C.RANfunctions_List_Vone_t, error) {
	rflC := new(C.RANfunctions_List_Vone_t)
...
func decodeRanFunctionsListBytes(ranFunctionListChoice [48]byte) (*e2appducontents.RanfunctionsList, error) {
	...
	ranFunctionListChoiceC := C.RANfunctions_List_Vone_t{
		list: C.struct___116{
			array: array,
			size:  size,
			count: count,
...
func decodeRanFunctionsList(rflC *C.RANfunctions_List_Vone_t) (*e2appducontents.RanfunctionsList, error) {
