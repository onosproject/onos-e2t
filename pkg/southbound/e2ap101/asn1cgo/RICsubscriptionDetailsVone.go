...
func newRicSubscriptionDetails(rsDet *e2appducontents.RicsubscriptionDetails) (*C.RICsubscriptionDetailsVone_t, error) {
	...
	rsDetC := C.RICsubscriptionDetailsVone_t{
...
func decodeRicSubscriptionDetailsBytes(bytes []byte) (*e2appducontents.RicsubscriptionDetails, error) {
	...
	rsDetC := C.RICsubscriptionDetailsVone_t{
		ricEventTriggerDefinition: C.RICeventTriggerDefinitionVone_t{
			buf:  (*C.uchar)(unsafe.Pointer(uintptr(binary.LittleEndian.Uint64(bytes[:8])))),
			size: C.ulong(binary.LittleEndian.Uint64(bytes[8:])),
		},
		ricAction_ToBeSetup_List: C.RICactions_ToBeSetup_List_Vone_t{
...
func decodeRicSubscriptionDetails(rsDetC *C.RICsubscriptionDetailsVone_t) (*e2appducontents.RicsubscriptionDetails, error) {
