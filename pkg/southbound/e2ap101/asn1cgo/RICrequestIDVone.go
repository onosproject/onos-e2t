...
func xerDecodeRicRequestID(bytes []byte) (*e2apies.RicrequestId, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_RICrequestIDVone)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	ricIndicationC := (*C.RICrequestIDVone_t)(unsafePtr)
	ricIndication := decodeRicRequestID(ricIndicationC)

	return ricIndication, nil
}

func newRicRequestID(rrID *e2apies.RicrequestId) *C.RICrequestIDVone_t {
	rrIDC := C.RICrequestIDVone_t{
...
func decodeRicRequestID(ricRequestIDCchoice *C.RICrequestIDVone_t) *e2apies.RicrequestId {
