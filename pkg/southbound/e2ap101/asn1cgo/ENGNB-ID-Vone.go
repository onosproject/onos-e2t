...
func xerEncodeEngnbID(engnbID *e2ap_ies.EngnbId) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_ENGNB_ID_Vone, unsafe.Pointer(engnbIDCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeEngnbID() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeEngnbID(engnbID *e2ap_ies.EngnbId) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_ENGNB_ID_Vone, unsafe.Pointer(engnbIDCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodeEngnbID() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodeEngnbID(bytes []byte) (*e2ap_ies.EngnbId, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_ENGNB_ID_Vone)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeEngnbID((*C.ENGNB_ID_Vone_t)(unsafePtr))
}

func perDecodeEngnbID(bytes []byte) (*e2ap_ies.EngnbId, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_ENGNB_ID_Vone)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newEngnbID(engnbID *e2ap_ies.EngnbId) (*C.ENGNB_ID_Vone_t, error) {

	var pr C.ENGNB_ID_Vone_PR
	choiceC := [48]byte{}

	switch choice := engnbID.EngnbId.(type) {
	case *e2ap_ies.EngnbId_GNbId:
		pr = C.ENGNB_ID_Vone_PR_gNB_ID
...
func decodeEngnbID(engnbIDC *C.ENGNB_ID_Vone_t) (*e2ap_ies.EngnbId, error) {

	engnbID := new(e2ap_ies.EngnbId)

	switch engnbIDC.present {
	case C.ENGNB_ID_Vone_PR_gNB_ID:
