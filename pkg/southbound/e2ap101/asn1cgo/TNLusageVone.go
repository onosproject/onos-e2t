...
func xerEncodeTnlusage(tnlusage *e2ap_ies.Tnlusage) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_TNLusageVone, unsafe.Pointer(tnlusageCP))
	if err != nil {
...
func perEncodeTnlusage(tnlusage *e2ap_ies.Tnlusage) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_TNLusageVone, unsafe.Pointer(tnlusageCP))
	if err != nil {
...
func xerDecodeTnlusage(bytes []byte) (*e2ap_ies.Tnlusage, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_TNLusageVone)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeTnlusage((*C.TNLusageVone_t)(unsafePtr))
}

func perDecodeTnlusage(bytes []byte) (*e2ap_ies.Tnlusage, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_TNLusageVone)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newTnlusage(tnlusage *e2ap_ies.Tnlusage) (*C.TNLusageVone_t, error) {
	var ret C.TNLusageVone_t
	switch *tnlusage {
	case e2ap_ies.Tnlusage_TNLUSAGE_RIC_SERVICE:
		ret = C.TNLusageVone_ric_service
	case e2ap_ies.Tnlusage_TNLUSAGE_SUPPORT_FUNCTION:
		ret = C.TNLusageVone_support_function
	case e2ap_ies.Tnlusage_TNLUSAGE_BOTH:
		ret = C.TNLusageVone_both
	default:
...
func decodeTnlusage(tnlusageC *C.TNLusageVone_t) (*e2ap_ies.Tnlusage, error) {
