...
func newRanFunctionIDCauseItem(rfIDi *e2appducontents.RanfunctionIdcauseItem) (*C.RANfunctionIDcause_Item_Vone_t, error) {
	cause, err := newCause(rfIDi.GetCause())
	if err != nil {
		return nil, fmt.Errorf("newCause() error %s", err.Error())
	}

	rfIDiC := C.RANfunctionIDcause_Item_Vone_t{
...
func decodeRanFunctionIDcauseItemBytes(rfic [72]byte) (*e2appducontents.RanfunctionIdcauseItem, error) {
	rficC := C.RANfunctionIDcause_Item_Vone_t{
		ranFunctionID: C.long(binary.LittleEndian.Uint64(rfic[:8])),
		cause: C.CauseVone_t{
			present: C.CauseVone_PR(binary.LittleEndian.Uint64(rfic[8:16])),
		},
	}
...
func decodeRanFunctionIDCauseItem(rfiC *C.RANfunctionIDcause_Item_Vone_t) (*e2appducontents.RanfunctionIdcauseItem, error) {
