...
func xerEncodegNBID(gnbID *e2apies.GlobalgNbId) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_GlobalgNB_ID_Vone, unsafe.Pointer(gnbIDC))
	if err != nil {
		return nil, err
	}
	return bytes, nil
...
func perEncodegNBID(gnbID *e2apies.GlobalgNbId) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_GlobalgNB_ID_Vone, unsafe.Pointer(gnbIDC))
	if err != nil {
		return nil, err
	}
...
func newGlobalgNBID(id *e2apies.GlobalgNbId) (*C.GlobalgNB_ID_Vone_t, error) {
...
func decodeGlobalGnbID(globalGnbID *C.GlobalgNB_ID_Vone_t) (*e2apies.GlobalgNbId, error) {
