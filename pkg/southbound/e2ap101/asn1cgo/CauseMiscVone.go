...
func xerEncodeCauseMisc(causeMisc *e2ap_ies.CauseMisc) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_CauseMiscVone, unsafe.Pointer(causeMiscCP))
	if err != nil {
...
func perEncodeCauseMisc(causeMisc *e2ap_ies.CauseMisc) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_CauseMiscVone, unsafe.Pointer(causeMiscCP))
	if err != nil {
...
func xerDecodeCauseMisc(bytes []byte) (*e2ap_ies.CauseMisc, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_CauseMiscVone)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeCauseMisc((*C.CauseMiscVone_t)(unsafePtr))
}

func perDecodeCauseMisc(bytes []byte) (*e2ap_ies.CauseMisc, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_CauseMiscVone)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newCauseMisc(causeMisc *e2ap_ies.CauseMisc) (*C.CauseMiscVone_t, error) {
	var ret C.CauseMiscVone_t
	switch *causeMisc {
	case e2ap_ies.CauseMisc_CAUSE_MISC_CONTROL_PROCESSING_OVERLOAD:
		ret = C.CauseMiscVone_control_processing_overload
	case e2ap_ies.CauseMisc_CAUSE_MISC_HARDWARE_FAILURE:
		ret = C.CauseMiscVone_hardware_failure
	case e2ap_ies.CauseMisc_CAUSE_MISC_OM_INTERVENTION:
		ret = C.CauseMiscVone_om_intervention
	case e2ap_ies.CauseMisc_CAUSE_MISC_UNSPECIFIED:
		ret = C.CauseMiscVone_unspecified
	default:
...
func decodeCauseMisc(causeMiscC *C.CauseMiscVone_t) (*e2ap_ies.CauseMisc, error) {
