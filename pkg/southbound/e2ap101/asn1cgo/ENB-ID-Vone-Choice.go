...
func xerEncodeEnbIDChoice(enbIDChoice *e2ap_ies.EnbIdChoice) ([]byte, error) {
	...
	bytes, err := encodeXer(&C.asn_DEF_ENB_ID_Vone_Choice, unsafe.Pointer(enbIDChoiceCP))
	if err != nil {
		return nil, fmt.Errorf("xerEncodeEnbIDChoice() %s", err.Error())
	}
	return bytes, nil
...
func perEncodeEnbIDChoice(enbIDChoice *e2ap_ies.EnbIdChoice) ([]byte, error) {
	...
	bytes, err := encodePerBuffer(&C.asn_DEF_ENB_ID_Vone_Choice, unsafe.Pointer(enbIDChoiceCP))
	if err != nil {
		return nil, fmt.Errorf("perEncodeEnbIDChoice() %s", err.Error())
	}
	return bytes, nil
...
func xerDecodeEnbIDChoice(bytes []byte) (*e2ap_ies.EnbIdChoice, error) {
	unsafePtr, err := decodeXer(bytes, &C.asn_DEF_ENB_ID_Vone_Choice)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
	...
	return decodeEnbIDChoice((*C.ENB_ID_Vone_Choice_t)(unsafePtr))
}

func perDecodeEnbIDChoice(bytes []byte) (*e2ap_ies.EnbIdChoice, error) {
	unsafePtr, err := decodePer(bytes, len(bytes), &C.asn_DEF_ENB_ID_Vone_Choice)
	if err != nil {
		return nil, err
	}
	if unsafePtr == nil {
...
func newEnbIDChoice(enbIDChoice *e2ap_ies.EnbIdChoice) (*C.ENB_ID_Vone_Choice_t, error) {

	var pr C.ENB_ID_Vone_Choice_PR
	choiceC := [48]byte{}

	switch choice := enbIDChoice.EnbIdChoice.(type) {
	case *e2ap_ies.EnbIdChoice_EnbIdMacro:
		pr = C.ENB_ID_Vone_Choice_PR_enb_ID_macro
	...
	case *e2ap_ies.EnbIdChoice_EnbIdShortmacro:
		pr = C.ENB_ID_Vone_Choice_PR_enb_ID_shortmacro
	...
	case *e2ap_ies.EnbIdChoice_EnbIdLongmacro:
		pr = C.ENB_ID_Vone_Choice_PR_enb_ID_longmacro
	...
	enbIDChoiceC := C.ENB_ID_Vone_Choice_t{
		present: pr,
		choice:  choiceC,
	}
...
func decodeEnbIDChoice(enbIDChoiceC *C.ENB_ID_Vone_Choice_t) (*e2ap_ies.EnbIdChoice, error) {

	enbIDChoice := new(e2ap_ies.EnbIdChoice)

	switch enbIDChoiceC.present {
	case C.ENB_ID_Vone_Choice_PR_enb_ID_macro:
	...
	case C.ENB_ID_Vone_Choice_PR_enb_ID_shortmacro:
		enbIDChoicestructC := newBitStringFromArray(enbIDChoiceC.choice)
		enbID, err := decodeBitString(enbIDChoicestructC)
		if err != nil{
			return nil, err
	...
	case C.ENB_ID_Vone_Choice_PR_enb_ID_longmacro:
		enbIDChoicestructC := newBitStringFromArray(enbIDChoiceC.choice)
		enbID, err := decodeBitString(enbIDChoicestructC)
		if err != nil{
			return nil, err
